...
const router = async function (config) {
  ...
  appRouter.param('document', async function (req, res, next, id) {
    ...
    if (id === 'admin' || id === 'undefined' || id === undefined) {
      req.session.error = 'Invalid document ID!';
      return res.redirect(res.locals.baseHref + 'db/' + req.dbName + '/' + req.collectionName);
    }

    // check if id is a string and not a json object
    if (typeof id === 'string' && id.startsWith('{') && id.endsWith('}')) {
      try {
        id = JSON.parse(id);
      } catch (error) {
        req.session.error = 'Invalid document ID format!';
        return res.redirect(res.locals.baseHref + 'db/' + req.dbName + '/' + req.collectionName);
      }
    }

    next()
  }, async function (req, res, next, id) {
